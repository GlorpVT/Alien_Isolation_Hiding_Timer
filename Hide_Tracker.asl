//////////////////////////////////////////////////////////////
//															//
//		 		Written and Tested by GlorpVT		    	//
//															//
//	  	  Written for the Community for Traking Hiding		//
//															//
//////////////////////////////////////////////////////////////
state("AI", "Steam")
{
	...
	byte PauseState : 0x05E8C38, 0x108;
	int StandState : 0x1358A94, 0x20, 0xB70;
    byte LoadingIcon : 0x0D4B9F0, 0x988, 0xE74;
}
state("AI", "Epic Games")
{
	...
	int PauseState : 0x1BDD1A0, 0x54, 0xAC0;
	int StandState : 0x0D53248, 0x14, 0x8F8, 0x54, 0x288;
}
state("AI", "Xbox Game Pass") 
{
    ...
    float Xaxis : 0x01918418, 0xC0;
    float Yaxis : 0x01918418, 0xC4;
	float Zaxis : 0x01918418, 0xC8;
}
startup // Contains Variable Declarations
{
	settings.Add("state", false, "Display Player State");
	settings.Add("pausing", true, "Count Pause/Map Screen");
	settings.Add("crouching", false, "Count Crouching");
	settings.Add("lockers", true, "Count Lockers");
	settings.Add("walking", false, "Count Walking");
	settings.Add("sprinting", false, "Count Sprinting");
...
update // Main Code
{
    if(settings["pausing"] == true && current.PauseState == 112){vars.loading = false;}                         // Pause/Map Screen
	if(current.PauseState == 160 || current.LoadingIcon == 1 || current.gameFlowState == 5){                    // Pauses timer on Load Screens
        vars.loading = true;
        if(settings["walking"] == true && current.StandState == 0){vars.loading = false;}                       // Walking Counter
        if(settings["sprinting"] == true && current.StandState == 1){vars.loading = false;}                     // Sprinting Counter
        if(settings["crouching"] == true && current.StandState == 2){vars.loading = false;}						// Crouched
        if(settings["crouching"] == true && current.StandState == 4){vars.loading = false;}						// Crouched Aiming
        if(settings["crouching"] == true && current.StandState == 19){vars.loading = true;}					    // In Vent	
        if(settings["crouching"] == true && current.StandState == 20){vars.loading = true;}                     // Aiming in Vent
        if(settings["crouching"] == true && current.StandState == 48){vars.loading = false;}					// Crouched With Tracker
        if(settings["crouching"] == true && current.StandState == 49){vars.loading = true;}                     // Tracker in Vent
        if(settings["state"] == true && current.StandState == 0){vars.State = "Walking";}		                  
        if(settings["state"] == true && current.StandState == 1){vars.State = "Sprinting";}		                                
        if(settings["state"] == true && current.StandState == 2){vars.State = "Crouched";}
        if(settings["state"] == true && current.StandState == 3){vars.State = "Aiming";}			                            
        if(settings["state"] == true && current.StandState == 4){vars.State = "Crouched Aiming";}
        if(settings["state"] == true && current.StandState == 19){vars.State = "In Vent";}
        if(settings["state"] == true && current.StandState == 20){vars.State = "Aiming in Vent";}						
        if(settings["state"] == true && current.StandState == 47){vars.State = "Tracker Out";}	                                
        if(settings["state"] == true && current.StandState == 48){vars.State = "Crouched w/ Tracker";}
        if(settings["state"] == true && current.StandState == 49){vars.State = "Tracker in Vent";}						
        if(settings["lockers"] == true){
            if(current.Map == "PRESENCE_HAB_AIRPORT")
                {
